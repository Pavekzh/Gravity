...
public class ArrowManipulator : ManipulatorView
{
    ...
    private void Start()
    {
        SetManipulator(originPoint,directPoint,touchPoint);
    }
    public override void SetManipulator(Vector3 originPoint, Vector3 directPoint, Vector3 touchPoint)
    {
        ...
        if (directPoint != originPoint)
        {
            ...
            line.transform.localScale = new Vector3(lineWidth * scaleFactor.x * zoomFactor, lineWidth * scaleFactor.y * zoomFactor, Vector3.Distance(directPoint, touchPoint) * scaleFactor.z);
            arrow.transform.localScale = arrowScale *  zoomFactor;
